...
const
  ...
  DefCurrencyName = 'тг';
  DefOfflineText = 'АВТОНОМНЫЙ РЕЖИМ';
  DefLineSpacing = 0;
...
begin
  if AppName = '' then
    raise UserException.Create('Название единицы приложения не может быть пустым');

  if FUnitNames.
  ItemByAppName(AppName) <> nil then
    raise UserException.CreateFmt('Соответствие для единицы "%s" уже задано', [AppName]);
