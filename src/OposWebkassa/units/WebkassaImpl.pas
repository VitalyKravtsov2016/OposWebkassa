...
begin
  ...
  try
    ...
    Document.Add('ŒÔÂ‡ÚÓ: ' + FCashier.FullName);
...
begin
  ...
  begin
    ...
    begin
      ...
      Document.AddLines('   —ÚÓËÏÓÒÚ¸', CurrencyToStr(RecItem.GetTotalWithDiscount));
    end;
    // Text
  ...
  if Amount <> 0 then
  begin
    Document.AddLines('—ÍË‰Í‡:', CurrencyToStr(Amount));
  end;
  // Õ‡ˆÂÌÍ‡ Ì‡ ˜ÂÍ
  Amount := Receipt.GetCharge;
  if Amount <> 0 then
  begin
    Document.AddLines('Õ‡ˆÂÌÍ‡:', CurrencyToStr(Amount));
  end;
  Document.AddLines('»“Œ√Œ:', CurrencyToStr(Receipt.GetTotal));
  if Receipt.Change <> 0 then
  begin
    Document.AddLines('  —ƒ¿◊¿:', CurrencyToStr(Receipt.Change));
  end;
  ...
  Document.AddSeparator;
  Document.AddLines('‘ËÒÍ‡Î¸Ì˚È ÔËÁÌ‡Í:',  Receipt.FiscalSign);
  Document.AddLines('¬ÂÏˇ:',  Command.Data.DateTime);
  Document.AddLines('ŒÔÂ‡ÚÓ ÙËÒÍ‡Î¸Ì˚ı ‰‡ÌÌ˚ı:',  Command.Data.Cashbox.Ofd.Name);
