...
begin
  ...
  PtrCheck(FPrinter.PrintNormal(PTR_S_RECEIPT, ESC_DoubleHighWide + 'Double high wide text' + CRLF));
  //PtrCheck(FPrinter.PrintBitmap(PTR_S_RECEIPT, 'QRCodeBitmap.bmp', PTR_BM_ASIS, PTR_BM_CENTER));
  PtrCheck(FPrinter.PrintBarCode(PTR_S_RECEIPT, 'Barcode', PTR_BCS_QRCODE,
    100, 100, PTR_BC_CENTER, PTR_BC_TEXT_NONE));

  FWinPrinter.Bitmap.SaveToFile('TestPrintReceipt.bmp');
end;
